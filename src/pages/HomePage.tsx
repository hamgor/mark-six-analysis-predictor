...
import { Activity, Database, Cpu, Zap, Globe, Server, Terminal } from 'lucide-react';
export function HomePage() {
  ...
  const marqueeText = "RETROSIX_ANALYST v2.0 // SYSTEM STATUS: NOMINAL // PROCESSING DRAW BATCHES: " + draws.length + " // ALGORITHM: TEMPORAL_DECAY_WEIGHTED_HEURISTICS // ADAPTIVE_INTELLIGENCE: ENABLED // ";
  return (
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-10 lg:py-12 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          <div className="lg:col-span-4 flex flex-col gap-8">
            ...
            <RetroCard title="MANUAL_UPLINK" headerColor="neon">
            ...
            <RetroCard title="KERNEL_STDOUT" className="h-[120px]" headerColor="ice">
              <div className="text-[9px] md:text-[10px] space-y-1.5 text-cyber-ice/50 font-mono leading-tight">
                <p>&gt; ADAPTIVE_ENGINE: {draws.length} NODES LOADED</p>
                <p>&gt; LOCAL_STORAGE_SYNC... [OK]</p>
                <p className="text-cyber-neon">&gt; STATUS: AWAITING_INPUT</p>
              </div>
            </RetroCard>
          </div>
          <div className="lg:col-span-8 flex flex-col gap-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="flex flex-col gap-8">
                <RetroCard title="VECTOR_DISTRIBUTION" headerColor="sapphire" className="h-[280px]">
                  <AnalysisCharts weightedFrequencies={weightedFrequencies} />
                </RetroCard>
                <RetroCard title="ANALYTIC_INSIGHTS" headerColor="ice" className="flex-1">
                  <RulesSummary 
                    stats={hotColdStats} 
                    pairs={frequentPairs} 
            ...
            <RetroCard className="py-4 px-8" headerColor="sapphire">
              <div className="flex flex-col md:flex-row justify-between items-center gap-4 text-[8px] md:text-[9px] text-cyber-ice/30 uppercase tracking-[0.4em] font-bold">
                <span className="flex items-center gap-2">
                  ...
                  RETROSIX_ANALYST // PHASE_4_DEPLOYED
                </span>
                <span className="hidden md:inline">ADAPTIVE_P_DISTRIBUTION: ACTIVE // VECTOR_POOL: 49</span>
                <span className="flex items-center gap-2">
                  <Globe size={10} />
